...
class TestDownloadFunctions:
    ...
    def test_download_run_data_with_datasets(
        self,
        mock_copytree,
        mock_mkdir,
        ...
        """Test the download_run_data function with datasets."""
        # Arrange
        ...
        with patch("cosmotech.coal.cosmotech_api.runner.download.RunnerApi", return_value=mock_runner_api):
            ...
            mock_write_parameters.assert_called_once_with(parameter_folder, mock_parameters, True, False)
    ...
    @patch("cosmotech.coal.cosmotech_api.runner.download.format_parameters_list")
    @patch("cosmotech.coal.cosmotech_api.runner.download.write_parameters")
    def test_download_run_data_no_datasets(
        self,
        mock_write_parameters,
        mock_format_parameters,
        mock_get_api_client,
    ):
        """Test the download_run_data function without datasets."""
        # Arrange
        organization_id = "org-123"
        workspace_id = "ws-123"
        runner_id = "runner-123"
    ...
    @patch("cosmotech.coal.cosmotech_api.runner.download.format_parameters_list")
    @patch("cosmotech.coal.cosmotech_api.runner.download.write_parameters")
    def test_download_run_data_fallback_to_scenario_api(
        self,
        mock_write_parameters,
        mock_format_parameters,
        mock_get_api_client,
    ):
        """Test the download_run_data function with fallback to scenario API."""
        # Arrange
        organization_id = "org-123"
        workspace_id = "ws-123"
        runner_id = "runner-123"
        ...
        # Mock runner API to raise exception
        mock_runner_api = MagicMock(spec=RunnerApi)
        mock_runner_api.get_runner.side_effect = ApiException(status=404, reason="Not Found")

        # Mock scenario API
        mock_scenario_api = MagicMock(spec=ScenarioApi)
        mock_runner_data = MagicMock()
        mock_runner_data.dataset_list = []
        mock_runner_data.parameters_values = [
            MagicMock(var_type="string", value="value1", parameter_id="param1"),
        ]
        mock_scenario_api.find_scenario_by_id.return_value = mock_runner_data
        ...
        with patch("cosmotech.coal.cosmotech_api.runner.download.RunnerApi", return_value=mock_runner_api):
            with patch("cosmotech.coal.cosmotech_api.runner.download.ScenarioApi", return_value=mock_scenario_api):
                # Act
                result = download_run_data(
                    organization_id=organization_id,
                    workspace_id=workspace_id,
                ...
                mock_scenario_api.find_scenario_by_id.assert_called_once_with(
                    organization_id=organization_id,
                    workspace_id=workspace_id,
