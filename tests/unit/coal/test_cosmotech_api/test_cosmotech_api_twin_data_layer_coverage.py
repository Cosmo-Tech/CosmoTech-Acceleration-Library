...
class TestTwinDataLayerCoverage:
    ...
    def test_get_dataset_id_from_runner_error_message(self, mock_get_api_client):
        """Test the error message in get_dataset_id_from_runner when both IDs are provided."""
        # Arrange
        ...
        assert "Requires a single Runner ID or Scenario ID to work" in str(excinfo.value)
        assert "Both were defined" in str(excinfo.value)

    @patch("cosmotech.coal.cosmotech_api.twin_data_layer.get_api_client")
    def test_get_dataset_id_from_runner_error_message_none(self, mock_get_api_client):
        """Test the error message in get_dataset_id_from_runner when no IDs are provided."""
        # Arrange
        mock_get_api_client.return_value = (MagicMock(), MagicMock())
        ...
        # Check the specific error message
        assert "Requires a single Runner ID or Scenario ID to work" in str(excinfo.value)
        assert "None were defined" in str(excinfo.value)
