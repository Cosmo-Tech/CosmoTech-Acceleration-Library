...
import pytest
import requests
from cosmotech_api import DatasetApi, RunnerApi, ScenarioApi, DatasetTwinGraphQuery
...
class TestTwinDataLayerFunctions:
    ...
    @pytest.fixture
    def mock_scenario_api(self):
        """Create a mock ScenarioApi."""
        mock_api = MagicMock(spec=ScenarioApi)
        return mock_api
    ...
    @patch("cosmotech.coal.cosmotech_api.twin_data_layer.get_api_client")
    def test_get_dataset_id_from_scenario(self, mock_get_api_client, mock_scenario_api):
        """Test the get_dataset_id_from_runner function with scenario_id."""
        # Arrange
        mock_get_api_client.return_value = (MagicMock(), MagicMock())
        mock_get_api_client.return_value[0].default_headers = {}
        mock_scenario = MagicMock()
        mock_scenario.dataset_list = ["dataset-456"]
        mock_scenario.id = "scenario-456"
        mock_scenario_api.find_scenario_by_id.return_value = mock_scenario

        with patch("cosmotech.coal.cosmotech_api.twin_data_layer.ScenarioApi", return_value=mock_scenario_api):
            # Act
            result = get_dataset_id_from_runner("org-123", "ws-123", scenario_id="scenario-456")

            # Assert
            assert result == "dataset-456"
            mock_scenario_api.find_scenario_by_id.assert_called_once_with("org-123", "ws-123", "scenario-456")

    @patch("cosmotech.coal.cosmotech_api.twin_data_layer.get_api_client")
    def test_get_dataset_id_from_runner_no_ids(self, mock_get_api_client):
    ...
    @patch("cosmotech.coal.cosmotech_api.twin_data_layer.get_api_client")
    def test_get_dataset_id_from_runner_both_ids(self, mock_get_api_client):
        """Test the get_dataset_id_from_runner function with both IDs."""
        # Arrange
        mock_get_api_client.return_value = (MagicMock(), MagicMock())

        # Act & Assert
        with pytest.raises(ValueError):
            get_dataset_id_from_runner("org-123", "ws-123", "runner-123", "scenario-456")
    ...
    def test_load_files_from_tdl(
        ...
        with patch("cosmotech.coal.cosmotech_api.twin_data_layer.DatasetApi", return_value=mock_dataset_api):
            ...
            # Assert
            mock_get_dataset_id.assert_called_once_with("org-123", "ws-123", "runner-123", None)
