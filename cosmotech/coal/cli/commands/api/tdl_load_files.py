...
def tdl_load_files(
    ...
    if (scenario_id is None) == (runner_id is None):
        LOGGER.error(
            "Requires a single Scenario ID or Runner ID to work."
    ...
    if (datasets_len := len(runner_info.dataset_list)) != 1:
        LOGGER.error(
            f"{runner_info.id} is not tied to a single dataset but {datasets_len}"
        )
    ...
    if dataset_info.ingestion_status != "SUCCESS":
        LOGGER.error(
            f"Dataset {dataset_id} is in state {dataset_info.ingestion_status}"
        )
    ...
    for element_type, query in item_queries.items():
        element_query: list[dict] = api_ds.twingraph_query(
