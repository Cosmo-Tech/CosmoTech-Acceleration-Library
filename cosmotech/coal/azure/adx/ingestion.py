...
def ingest_dataframe(
    ...
    """
    LOGGER.debug(T("coal.logs.adx.ingesting_dataframe").format(
...
def send_to_adx(
    ...
    Returns:
        The ingestion result with source_id for status tracking
    """
    LOGGER.debug(T("coal.logs.adx.sending_to_adx").format(
        table_name=table_name,
        items=len(dict_list)
    ))
...
def check_ingestion_status(
    ...
    for source_id in source_ids:
        ...
        if _ingest_status[source_id] not in [IngestionStatus.QUEUED, IngestionStatus.UNKNOWN]:
    ...
    if logs:
        LOGGER.debug(T("coal.logs.adx.status_messages").format(
        ...
        for _q, _m in messages:
            ...
            for source_id in to_check_ids:
                if dm.IngestionSourceId == str(source_id):
                    ...
                    if logs:
                        LOGGER.debug(T("coal.logs.adx.status_found").format(
                            source_id=source_id,
                            status=status.value
                        ))
