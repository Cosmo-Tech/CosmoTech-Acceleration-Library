...
def convert_dataset_to_files(dataset_info: Dict[str, Any], target_folder: Optional[Union[str, Path]] = None) -> Path:
    ...
    LOGGER.info(T("coal.logs.dataset.converting_to_files").format(
...
def convert_graph_dataset_to_files(content: Dict[str, List[Dict]], target_folder: Union[str, Path]) -> Path:
    ...
    for entity_type, entities in content.items():
        ...
        LOGGER.debug(T("coal.logs.dataset.writing_csv").format(
    ...
    LOGGER.info(T("coal.logs.dataset.files_created").format(
        count=file_count,
        folder=target_folder
    ))
...
def convert_file_dataset_to_files(content: Dict[str, Any], target_folder: Union[str, Path], file_type: str) -> Path:
    ...
    for file_name, file_content in content.items():
        ...
        LOGGER.debug(T("coal.logs.dataset.writing_file").format(
            file_name=file_path.name,
            file_type=file_type
        ))
