...
def download_file_dataset(
    ...
    with get_api_client()[0] as api_client:
        ...
        existing_files = list(
            _f.file_name for _f in all_api_files if _f.file_name.startswith(file_name)
        )
        list_time = time.time() - list_start

        LOGGER.debug(
            T("coal.logs.dataset.workspace_files_found").format(
                count=len(existing_files)
            )
        )
        LOGGER.debug(
            T("coal.logs.dataset.operation_timing").format(
                operation="file listing", time=list_time
            )
        )

        if not existing_files:
            LOGGER.warning(
                T("coal.logs.dataset.no_files_found").format(file_name=file_name)
            )
        ...
        for _file_name in existing_files:
            ...
            LOGGER.debug(
                T("coal.logs.dataset.downloading_file").format(file_name=_file_name)
            )
            ...
            LOGGER.debug(
                T("coal.logs.dataset.file_downloaded").format(
                    file_name=_file_name, path=target_file
                )
            )
            LOGGER.debug(
                T("coal.logs.dataset.operation_timing").format(
            ...
            if ".xls" in _file_name:
                LOGGER.debug(
                    T("coal.logs.dataset.processing_excel").format(
                ...
                for sheet_name in wb.sheetnames:
                    ...
                    LOGGER.debug(
                        T("coal.logs.dataset.sheet_processed").format(
                            sheet_name=sheet_name, rows=row_count
                        )
                    )

            elif ".csv" in _file_name:
                LOGGER.debug(
                    T("coal.logs.dataset.processing_csv").format(file_name=target_file)
                )
                with open(target_file, "r") as file:
                    ...
                    LOGGER.debug(
                        T("coal.logs.dataset.csv_processed").format(
                            file_name=current_filename, rows=row_count
                        )
                    )

            elif ".json" in _file_name:
                LOGGER.debug(
                    T("coal.logs.dataset.processing_json").format(file_name=target_file)
                )
                with open(target_file, "r") as _file:
                    ...
                    LOGGER.debug(
                        T("coal.logs.dataset.json_processed").format(
                            file_name=current_filename, items=item_count
                        )
                    )

            else:
                LOGGER.debug(
                    T("coal.logs.dataset.processing_text").format(file_name=target_file)
                )
                with open(target_file, "r") as _file:
                    current_filename = os.path.basename(target_file)
                    ...
                    LOGGER.debug(
                        T("coal.logs.dataset.text_processed").format(
                            file_name=current_filename, lines=line_count
                        )
                    )
            ...
            LOGGER.debug(
                T("coal.logs.dataset.operation_timing").format(
                    operation=f"process {_file_name}", time=process_time
                )
            )
    ...
    LOGGER.info(
        T("coal.logs.dataset.operation_timing").format(
            operation="File download", time=elapsed_time
        )
    )
