...
def get_dataset_id_from_runner(
    organization_id: str, workspace_id: str, runner_id: Optional[str] = None, scenario_id: Optional[str] = None
) -> str:
    """
    Get the dataset ID from a runner or scenario.

    Args:
        organization_id: Organization ID
        workspace_id: Workspace ID
        runner_id: Runner ID (optional)
    ...
    Raises:
        ValueError: If neither runner_id nor scenario_id is provided, or if both are provided
    """
    if (runner_id is None) == (scenario_id is None):
        raise ValueError(
            "Requires a single Runner ID or Scenario ID to work. "
            + ("Both" if runner_id else "None")
            + " were defined."
        )
...
def load_files_from_tdl(
    ...
    # Get dataset ID from runner or scenario
    dataset_id = get_dataset_id_from_runner(organization_id, workspace_id, runner_id, scenario_id)
