...
class CSVSourceFile:
    def __init__(self, file_path: pathlib.Path):
        ...
        if not file_path.name.endswith(".csv"):
            raise ValueError(
                T("coal.errors.validation.not_csv_file").format(file_path=file_path)
            )
        ...
        if not has_id and not is_relation:
            LOGGER.error(
                T("coal.errors.validation.invalid_nodes_relations").format(
                    file_path=file_path
                )
            )
            LOGGER.error(
                T("coal.errors.validation.node_requirements").format(
                    id_column=ID_COLUMN
                )
            )
            LOGGER.error(
