...
"""
Orchestration functions for downloading runner and scenario data.
"""
...
from cosmotech.coal.scenario.parameters import format_parameters_list, write_parameters
...
def download_scenario_data(
    ...
    with get_api_client()[0] as api_client:
        ...
        try:
            ...
            # Fallback for Scenario API in case of use of older API version
            api_instance = ScenarioApi(api_client)
            scenario_data = api_instance.find_scenario_by_id(
                organization_id=organization_id,
                workspace_id=workspace_id,
    ...
    # Format parameters
    parameters = format_parameters_list(scenario_data)
    ...
    if fetch_dataset:
        ...
        if dataset_ids:
            ...
            # Save datasets to parameter folders and dataset folder
            for dataset_id, dataset_info in datasets.items():
                # If dataset is in dataset_list and dataset_folder is provided, save there
                if dataset_folder and dataset_id in scenario_data.dataset_list:
                    ...
                    LOGGER.debug(T("coal.logs.scenario.dataset_debug").format(folder=dataset_folder, id=dataset_id))
                ...
                if dataset_id in datasets_parameters_ids:
                    ...
                    LOGGER.debug(T("coal.logs.scenario.dataset_debug").format(folder=param_id, id=dataset_id))
    else:
